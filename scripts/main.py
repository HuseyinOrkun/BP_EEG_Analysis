...
# %%
# Get all .set files that end with preprocessed.set
...
for preprocessed_filepath in preprocessed_filepath_list:
    ...
    autorejected_epochs_file = \
        cfg.subject_processed_file_path \
    ...
    if os.path.exists(autorejected_epochs_file) and autoreject:
        ...
        plt.savefig(
            cfg.subject_output_folder /
            f"{cfg.current_subject}_autoreject-plot.png")

    elif not os.path.exists(autorejected_epochs_file) and autoreject:
        epochs = bp_io.read_eeglab_preprocessed_file()
        original_epochs = epochs.copy()
        epochs = erp_analysis.autoreject_analysis(epochs)
    else:
        epochs = bp_io.read_eeglab_preprocessed_file()
        original_epochs = epochs.copy()
